...
def get_secret_api_key() -> str:
    """Streamlit secrets에서 API key를 읽는다. 로컬 secrets 파일이 없어도 앱이 죽지 않도록 처리."""
    try:
...
def fetch_monthly_adjusted(symbol: str, api_key: str) -> pd.DataFrame:
    ...
    if "Information" in data:
        raise ValueError(f"{symbol}: Alpha Vantage 안내 메시지 - {data['Information']}")
...
with st.sidebar:
    ...
    api_key = st.text_input(
        ...
        value=secret_key,
        type="password",
        help="Streamlit Secrets에 ALPHA_VANTAGE_API_KEY를 저장하면 자동으로 불러옵니다.",
    )
...
# =========================================================
if not api_key:
    st.warning("Alpha Vantage API Key를 입력하거나 Streamlit Secrets에 저장해주세요.")
    st.stop()
...
if run:
    ...
    prices = build_price_matrix(data, ALL_TICKERS, exclude_current_month=exclude_current_month)
    ...
    c1.metric("사용 ETF 수", f"{len([t for t in ALL_TICKERS if t in prices.columns])}개")
    ...
    st.markdown(
        """
        ### 사용 방법
        1. 왼쪽 사이드바에 Alpha Vantage API Key를 입력합니다.
