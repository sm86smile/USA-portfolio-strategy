...
c_api.metric("API 호출 대상", f"{len(DATA_TICKERS)}개 ETF + USD/KRW 환율")
...
st.info(
    ...
    f"이번 실행의 API 호출 대상은 {', '.join(DATA_TICKERS)} 및 USD/KRW 일별 환율입니다."
)
...
if run:
    ...
    detail_display = money_cols(detail_display, ["목표 투자금(KRW)"])
    detail_display = usd_money_cols(detail_display, ["목표 투자금(USD)"])
    ...
    final_display = money_cols(final_display, ["목표 투자금(KRW)"])
    final_display = usd_money_cols(final_display, ["목표 투자금(USD)"])
    ...
    st.info("배분 기준 화폐와 총 투자금, 조회 기준, LAA 조건, 최근 리밸런싱일, 하위전략 비중을 선택한 뒤 '전략 비중 계산'을 누르세요.")
